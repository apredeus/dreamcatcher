...
if (-d $dir."/output" && -e $dir."/Log.final.out") { 
  ...
  close READ1;
  close READ2;
  print STDOUT "make_umi_fastq.pl: making UMI-tools formatted reads: outputted $passed reads, discarded $discarded reads because of extensive homopolymers ..\n"; 
} else { 
  ## here we assume it's a Cell/Space Ranger/STARsolo-formatted BAM file instead of the directory 
  ## single- or paired-end (meaning biological reads), with CB/UB set for all reads that have matched against a barcode (and thus are error-corrected). 
  ## we will take only unmapped reads (-f4), and only R2 for paired-end reads (-f128)
  my $bam = $dir;
  if ($bam =~ m/\.bam$/) { 
    print STDOUT "make_umi_fastq.pl: sample appears to be to be Cell/Space Ranger/STARsolo BAM! \nMaking a single-end, UMI-tools-formatted fastq file..\n";
  } else { 
  ...
  ## we need to check if the file was mapped as paired-end; this changes what R1 and R2 actually are.
  ## if $paired == 1, flags 64/128 will be set (R1 will be trimmed for BC+UMI in a typical 10x Chromium 5' PE BAM).  
  my $preads = `samtools view -h $bam | head -10000 | samtools flagstat - | grep \"properly paired\" | cut -d' ' -f1`; 
  my $paired = ($preads > 0) ? 1 : 0; 
  if ($paired) { 
    print STDOUT "make_umi_fastq.pl: sample was determined to be 5' 10x paired-end experiment!\n";
  } else { 
    print STDOUT "make_umi_fastq.pl: sample was determined to be 3'/5'/Visium 10x single-end experiment!\n"; 
  } 
  ...
  while (<BAM>) { 
    ...
    if (m/CB:\w:([A-Z]+).*UB:\w:([A-Z]+)/) { 
      $bc = $1;
      $umi = $2;
    } elsif (m/CR:\w:([A-Z]+).*UR:\w:([A-Z]+)/) {
      $bc = $1; 
      $umi = $2; 
    } else { 
      print STDERR "make_umi_fastq.pl: WARNING - no barcode/umi tag found for read $rname! Please investigate..\n"; 
    } 

    ## chech if read has too much homopolymers 
    if (! low_complexity($seq)) { 
      printf OUT "%s_%s_%s\n%s\n+\n%s\n",$rname,$bc,$umi,$seq,$qual;
      $passed++; 
    } else {
