...
then
    ...
    mv barcode_bowtie2.log barcodes_to_correct.fa barcodes_vs_whitelist.tsv whitelist.fa Unmapped_filt.R?.fastq.gz 0_preprocessed_reads/
    mv kuniq* 1_krakenuniq/
    mv detected* read_to_gene_detected_w_mismatches.tsv gene_biotypes.tsv 2_detected_strains/
    ...
    mv prefix_to_species.tsv umi_tools_count.log bac_umi_counts.tsv.gz barcode_to_species.tsv 6_umitools/
    ...
    mv Unmapped_filt.R?.fastq.gz 0_preprocessed_reads/
    mv kuniq* 1_krakenuniq/
    mv detected* read_to_gene_detected_w_mismatches.tsv gene_biotypes.tsv 2_detected_strains/
    mv filter* blacklisted_genes.list hisat2_human.log human_remap.bam read_* 3_filtered_strains/
    mv rRNA.* protein.* nodes_and_edges.tsv multi_filt_reads.list unique_filt_reads.list reads_by_uniqness.tsv 4_read_networks/
