...
if (scalar @ARGV != 3) { 
	print "USAGE: ./report_to_GCF.pl <KrakenUniq_report> <gcf_species_genus_table> <genbank_acc_file>\n"; 
	exit 1; 
} 
...
while (<GCF2TAXID>) {
	...
	if ($st ne "NONE" && $gt ne "NONE") {
		printf STDERR "WARNING: Species taxid according to Genbank (%d) and according to NCBI taxDB (%d) differ!\n",$META->{$gcf}->{st},$st if ($META->{$gcf}->{st} != $st); 
		push @{$GSA->{$gt}->{$st}},$gcf;
		$META->{$gt}->{name} = $t[4]; 
		$META->{$st}->{name} = $t[2]; 
	} 
...
while (<REPORT>) {
	if (m/\t2\tsuperkingdom\t\s+Bacteria/) { 
	  ...
	  my @t = split /\t/;
		...
		## TODO: fix the cases when species do not have a genus (happens most often) 
		if ($rank eq "genus") { 
			$gt = $t[6]; 
			$genus_spaces = $spaces; 
		  if (! defined $META->{$gt}) {
        ## this really shouldn't happen 
			  printf STDERR "WARNING: Genus with taxid %d was not defined in NCBI/Genbank files!\n",$gt;
		  } else {
			  $META->{$gt}->{kmers} = $kmers;
   		} 
		} 
		if ($rank eq "species" && $spaces > $genus_spaces) { 
		  ...
		  if (! defined $META->{$st}) {
        ## this really shouldn't happen 
			  printf STDERR "WARNING: Species (%d) / genus(%d) pair was not defined in NCBI/Genbank files!\n",$st,$gt;
		  } else {
			  $META->{$st}->{kmers} = $kmers;
   		} 
		} 
		if ($rank eq "assembly" && $spaces > $species_spaces) { 
		  ...
		  if (! defined $META->{$gcf}) {
        ## this really shouldn't happen 
			  printf STDERR "WARNING: Assembly %s was not defined for species %d / genus %d in NCBI/Genbank files!\n",$gcf,$st,$gt;
		  } else {
