...
if (scalar @ARGV != 4) { 
  print STDERR "Usage: ./make_annotated_tables.pl <fcounts_tsv> <gene_type_tsv> <annotated_reads_tsv> <seq2taxid.map>\n"; 
  exit 1; 
} 
...
open ANNFC,">","annotated.fcounts.tsv"; 
...
print STDOUT "Reading gene/gene_type relationship table..\n"; 
while (<TYPES>) {
  chomp; 
  my @t = split /\t/; 
  $GENE->{$t[0]}->{type} = $t[1];
...
print STDOUT "Reading seq2taxid table..\n";
while (<SEQ2TAXID>) {
  chomp;
  my @t = split /\t/;
  $t[2] =~ m/(GCF_\d+\.\d+) (.*?)$/;
...
print STDOUT "Reading read table, counting mismatches per gene..\n"; 
while (<READS>) { 
  chomp; 
  my @t = split /\t/;
  if ($t[1] eq "-") { 
...
print STDOUT "Annotating the featureCounts output..\n"; 
while (<FCOUNTS>) { 
  chomp;
  next if (m/^#/ || m/^Geneid/); 
  my @t = split /\t/; 
