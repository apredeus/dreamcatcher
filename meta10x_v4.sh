...
$FCDIR/featureCounts -T $CPUS -M -O --fraction -t gene -g locus_tag -s 0 -a $TAG.combined.gff -o $TAG.fcounts.tsv $TAG.combined.bam
...
echo "STEP 9: Annotating the featureCounts table and generating summary per RefSeq accession (with at least 1 mapped read) .." 
./annotate_fc_table.pl $TAG.fcounts.tsv gene_biotypes.tsv $KUDB/seqid2taxid_v2.map
...
grep -wF -f filtered.acc.list $KUDB/seqid2taxid_v2.map | cut -f1 > filtered.chr.list
samtools view -@$CPUS $TAG.combined.bam | grep -wF -f filtered.chr.list | cut -f1,3 | sort | uniq > read_to_chr_filtered_uniq.tsv

./make_read_network.pl read_to_chr_filtered_uniq.tsv $KUDB/seqid2taxid_v2.map > nodes_and_edges.tsv

## step 12: parse the network using igraph
echo "STEP 12: Parsing read network using igraph .." 
Rscript parse_read_network.R 
