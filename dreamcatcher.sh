...
samtools view -@$CPUS human_remap.bam | perl -ne '@t=split/\t/; print "$t[0]\t$1\n" if (m/NM:i:(\d+)/)' | sort | uniq > human_read_to_mismatch.tsv
...
./annotate_mapped_reads.pl combined_bacterial.bam.featureCounts.bam human_read_to_mismatch.tsv > combined_bacterial_reads_annotated.tsv
